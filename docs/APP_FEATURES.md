...
| Map View | `body.map`, `#map`, `#pac-input` | Interactive Google Maps state for search, location selection, encoding, and viewing decoded places. Bottom-left chrome shows Info Action immediately before Wolo Code Input Action. **Show labels** from Info Modal View names those controls, Account, Locate, and Switch map over an 80% dark scrim on the map. |
