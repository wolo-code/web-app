...
| Map View | `body.map`, `#map`, `#pac-input` | Interactive Google Maps state for search, location selection, encoding, and viewing decoded places. Bottom-left chrome shows Info Action immediately before Wolo Code Input Action. **Show labels** from Info Modal View names those controls, Account, Locate, Switch map, Place Search Input, Map Proceed Button, and Camera D-pad over an 80% dark scrim on the map. Search/Go captions are pinned above the scrim from the visible search field so they are not trapped in the map stacking context. |
