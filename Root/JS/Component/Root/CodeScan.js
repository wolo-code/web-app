...
function openCodeScan(event) {
	...
	if(!codeScanOcrMatch.isCodeScanSupported()) {
		showCodeScanStatus('Camera scanning is not supported in this browser. Type or paste your Wolo Code instead.');
...
function handleCodeScanCameraError(error) {
	var message = 'Camera permission denied. Type or paste your Wolo Code instead.';
	if(error && error.name === 'NotFoundError')
		message = 'No camera found. Type or paste your Wolo Code instead.';
	else if(error && error.name === 'NotAllowedError')
		message = 'Camera permission denied. Type or paste your Wolo Code instead.';
	else if(error && error.name === 'NotReadableError')
		message = 'Camera is unavailable. Type or paste your Wolo Code instead.';
...
function handleCodeScanOcrResult(result) {
	...
	if(!text || confidence < CODE_SCAN_MIN_CONFIDENCE) {
		setCodeScanStatus('Looking for a Wolo Code label...');
		setCodeScanCandidate('');
		resetCodeScanMatchState();
		return;
	}
	...
	if(!match) {
		setCodeScanStatus('No valid Wolo Code found yet. Hold the label steady.');
		setCodeScanCandidate('');
		resetCodeScanMatchState();
		return;
	}
...
function applyScannedWoloCode(code) {
	...
	if(typeof syncProceedButtons == 'function')
		syncProceedButtons();
	if(typeof beginDecode == 'function')
		beginDecode(code);
}
