...
| Map Guide search restoration | Map Search Cluster Restoration | `decodeIconGuideSearchHome.style`, `restoreMapSearchBarStacking()` | Google Maps top-left control position on wide and narrow layouts | Preserves the original search cluster's complete SDK-provided inline style before the guide lifts it, then restores that exact style when the guide closes so its vertical position does not shift. |
