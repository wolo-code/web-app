...
| Decode Wolo Code | `decode()`, `beginDecode()`, `decode_input_from_form()`, `showInvalidCodeDialog()` | A typed Wolo Code, DIGIPIN, or plus code resolves to a place and can jump back to the map. DIGIPIN and plus-code jumps keep the Address Panel open. Unrecognized input opens a popup to correct the code or search the map for that text instead of a notification toast. Wolo Code Input and Place Search Input use CSS `text-transform` to show DIGIPIN and plus-code values in uppercase (`:valid` against those patterns) and return to lowercase when the value no longer matches. |
| Decode input hint | `#decode_input_alt_tip` | While the Wolo Code input is focused, a small tip directly below the field notes that DIGIPIN and plus codes can also be entered. |
