...
| Label scan dialog | Wolo Code Scan Dialog | `#code_scan_message`, `Code_scan.php`, `CodeScan.js` | Center overlay when opened | Live preview with guidance-only OCR, Fixed 3:1 / General modes, shutter, and latched single capture. Review: frozen still, editable city + three words, city chooser, live validity, **Use Code** → `decode_input_from_form()`. **Rescan** restarts live keeping mode; **Cancel** dismisses without decode. Photo fallback uses the same review step. |
