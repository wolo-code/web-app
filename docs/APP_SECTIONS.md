...
| Accuracy strip | Location Accuracy Indicator | `#accuracy_container`, `#wait_loader` | Inside `#map_bottom_stack`, above the Address Panel when both are open | Shows active location accuracy feedback while geolocation is running. The top bar loader shows until the first GPS fix, then hides as the map zooms to the accuracy circle. |
