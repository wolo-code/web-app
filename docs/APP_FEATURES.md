...
| Unexpected Error Dialog | `#exception_message`, `showErrorPrompt()` | Blocking crash prompt titled `Oops an error occured!` with a warning icon. No close control or title tooltip. User actions are Clear cache & reload, then a crawl-proof `support@wolo.codes` contact on the next line after “You may contact our support team.” Press and hold the title to show the crash message and a Continue bypass for development. |
