...
| Feature | Current implementation signal | Notes |
| --- | --- | --- |
| Decode View | `body.decode`, `#decode_interface_overlay`, `#decode_input` | Plain Wolo Code entry state. Users can type or paste a Wolo Code before switching into the map flow. The Terrain Map View Button always uses `Map-terrain.svg`, even when the affixed default source is OSM, Apple Maps, Esri, or Microsoft Maps. In dark mode, the bottom-left Info Action, center Locate Button, and right Terrain Map View Button share the same circular disc. The first two app launches show icon captions over the Wolo Code Input View background (`#decode_icon_guide_scrim`, `body.decode-icon-guide`), not the map. Light mode keeps that background; dark mode adds an 80% dark scrim. A tap anywhere or a 3-second timeout fades the overlay away. Info Modal View can reopen those labels with the bottom-left **show guide** button on the current view, including Map View. Captions stay with the vertically centered input cluster on wide and mobile layouts; on very short screens the cluster shifts up so it clears bottom notifications. |
| Map View | `body.map`, `#map`, `#pac-input` | Interactive Google Maps state for search, location selection, encoding, and viewing decoded places. The App Logo plate (`#logo_surface`) keeps its frosted background on map views in light and dark mode; Wolo Code Input View in dark mode still omits that plate. Opening Map View (and later resize) raises zoom to a viewport-fill minimum so the world map covers the screen with no blank bands, including on tall phones when the camera is off the equator; users cannot zoom out below that floor. Bottom-left chrome shows Info Action immediately before Wolo Code Input Action. **show guide** from Info Modal View names those controls, Account, Locate, Switch map, Place Search Input, Map Proceed Button, and Camera D-pad, and shows a sample Location Wolo Label View (`Wolo-infocard-sample.svg`) with larger callout titles, italic short descriptions, and leader lines that end in circle dots on City, Wolo Code, Address, Open, and Label. The map-local dimmer uses `World-map.svg` over the Wolo Code Input View background: light mode keeps the original teal-on-pale-grey land, and dark mode uses the brand accent on that same dark input-view fill. OSM still uses that input-view fill rather than an 80% dark wash. Search/Go captions are pinned above the scrim from the visible search field without watching live map DOM mutations, so the tab stays responsive. Wolo Code sits to the right of its icon; D-pad sits to the left of the camera control. The search bar, Go button, and camera control stay above a map-local dimmer. Search and Go are lifted above the page scrim so they stay readable, with a solid field and a 1px accent edge rather than a heavier outline. A tap on the map or chrome, or the 3-second timeout, fades the overlay. |
...
| Current Location | `#location_button`, locate permission dialog, `#accuracy_container`, `#proceed_button` | Location control can request permission, locate the user, and optionally remember "do not ask again". The Location Accuracy Indicator is a 32px pill: the FF proceed control is flush with the pill (no raised shadow) and vertically centered, and the strip keeps a fixed width and item positions while the meter updates (`99+` slot) and after proceed hides the FF control without collapsing its slot. Five consecutive `99+` samples auto-proceed like FF, except in long-press locate override, which keeps watching and does not auto-proceed. |
...
| Overlay dismiss | `#overlay`, `onOverlayBackdropClick()` | Clicking the dimmed overlay outside a modal closes it, using that dialog’s close or cancel control when present. Unexpected Error Dialog stays open. |
