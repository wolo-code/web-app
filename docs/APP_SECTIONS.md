...
| Accuracy strip | Location Accuracy Indicator | `#accuracy_container`, `#wait_loader` | Inside `#map_bottom_stack`, above the Address Panel when both are open | Shows active location accuracy feedback while geolocation is running. The first GPS fix zooms the map to the accuracy circle, and the top bar loader stays visible so it is clear the locate watch will end. |
...
| Version indicator | Info Version Indicator | `#info_version_indicator`, `.info_version_stamp`, `toggleInfoVersionDisplay()` | Bottom-right of Info Modal View | Shows `v` plus the three-part `version` from `Vars.tsv` on the same bottom line as **show guide**. UTC and elapsed stamps overlay above that line and local time overlays below it so the version stays put. Hover or keyboard focus expands the label to `version.build` and shows the UTC timestamp above plus local time below. A tap keeps the full version visible, shows elapsed time since that bake in place of the UTC stamp, and keeps local time under the version. Hidden on Info Links View. |
