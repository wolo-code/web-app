...
| Map Guide search restoration | Map Search Cluster Restoration | `decodeIconGuideSearchHome.style`, `restoreMapSearchBarStacking()` | Google Maps top-left control position on wide and narrow layouts | Waits until Google Maps reports an on-screen position that has reached the cluster's computed responsive top margin before the first lift, rejecting the temporary wide offset on narrow screens. It preserves the complete SDK-provided inline style and restores that exact style when the guide closes. The dark-mode guide D-pad clone matches the live camera control's dark surface and accent icon. |
...
| Unexpected error dialog | Unexpected Error Dialog | `#exception_message`, `#exception_message_title`, `#exception_log`, `#exception_message_support` | Center overlay | Blocking crash prompt with a warning icon and `Oops an error occured!` title. No close button. First shows Clear cache & reload; the support email line appears only after that button was pressed and the crash returns. A successful load hides the contact again. Press and hold the title to show the crash message and Continue. Clicking the dimmed overlay does not close it. |
