...
| Map Guide search restoration | Map Search Cluster Restoration | `decodeIconGuideSearchHome.style`, `restoreMapSearchBarStacking()` | Google Maps top-left control position on wide and narrow layouts | Waits until Google Maps reports a non-zero on-screen position before the first lift, preserves the original search cluster's complete SDK-provided inline style, then restores that exact style when the guide closes so its vertical position does not shift. The dark-mode guide D-pad clone matches the live camera control's dark surface and accent icon. |
