...
| Label scan dialog | Wolo Code Scan Dialog | `#code_scan_message`, `Code_scan.php`, `CodeScan.js` | Center overlay when opened | Live rear-camera preview while Scan is active, processed on-device with Tesseract.js. Success fills `#decode_input` and calls `decode_input_from_form()`. Fallbacks: **Use photo instead** (`#code_scan_photo_input`, still local OCR), **Type instead**, or close and type/paste manually. |
