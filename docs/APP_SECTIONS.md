...
| Accuracy strip | Location Accuracy Indicator | `#accuracy_container`, `#wait_loader` | Inside `#map_bottom_stack`, above the Address Panel when both are open | Shows active location accuracy feedback while geolocation is running. The top bar loader stays continuous across initial bootstrap and IP-city retry attempts, and during location work it shows until the first GPS fix before hiding as the map zooms to the accuracy circle. |
