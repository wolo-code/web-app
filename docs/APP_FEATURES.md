...
| Default Try Prompt | `#notification_top`, `tryDefaultCity()` | Top notification offers a starter city example. Choosing it pans to the sample Bengaluru Wolo Code and animates zoom-in to street level the same way locate/decode focus does. |
...
| Feature | Current implementation signal | Notes |
| --- | --- | --- |
| Info Dialog | `Info.php`, `Info_intro.php`, `Info_full.php`, `Info_links.php`, `#info_show_icon_labels`, `#info_version_indicator` | Explains Wolo Code usage and links. The overlay card is vertically centered. In dark mode the Info Brand lockup has no light outline. A **Show labels** button in the bottom-left corner closes the dialog and shows the Input Icon Guide overlay on the current view (Wolo Code Input View or Map View), including after the first-launch captions have already been used. Hover and keyboard focus do not underline that button. The bottom-right version label and Info Links View version line show the three-part `version` from `Root/Config/Vars.tsv`. The corner control always reserves space for the bake timestamp above the version. A tap expands the label to `version.build` without shifting layout; hover or keyboard focus shows the timestamp in local time. Production merge (`render-native.ps1` / `render.sh`) increments `build` before baking so each deploy gets a new number. |
