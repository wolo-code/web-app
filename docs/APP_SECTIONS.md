...
| Locate proceed control | Location Proceed Button | `#proceed_button` | Right side of Location Accuracy Indicator, flush with the pill | Lets the user proceed with the current location fix. The control is vertically centered in the 32px pill, flattened onto the strip (no separate raised layer), and keeps its layout slot after it is hidden so the strip does not resize. Five consecutive `99+` samples also proceed automatically unless long-press locate override is on (`locate_button_pressed`). |
