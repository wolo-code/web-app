...
| Terrain map page | Terrain Map View | `body.map`, Google Maps `ROADMAP`, `#map`, `#pac-input` | Primary map interaction state for searching places, selecting points, encoding locations, and viewing decoded results on the standard Google map. Camera min zoom is the smallest level that still fills the map height so poleward gray bars do not appear. |
...
| Add city dialog | Add City Modal View | `#no_city_message`, `NoCity.php` | Overlay when a selected location is not in the city database. Follows light and dark theme, including the wait spinner. |
...
| City source controls | Input City Source Controls | `#decode_city_geolocation`, `#decode_city_ip`, `#decode_city_history_toggle`, `#decode_code_scan_button` | Above Input City Hint | Lets the user request coarse geolocation, switch to IP city, open the previous-city popup, or scan a printed label with on-device OCR using action-sized, widely spaced controls whose full circular button grows slightly on hover, active, or opening states and animates back when inactive. The selected source uses the primary icon accent; inactive sources stay muted. Unsupported, invalid, or non-gesture geolocation attempts use the standard city lookup failure state. Scan requests a new camera permission only while the scan dialog is open. |
| Label scan dialog | Wolo Code Scan Dialog | `#code_scan_message`, `Code_scan.php`, `CodeScan.js` | Center overlay when opened | Live rear-camera preview with Fixed 3:1 / General modes, shutter capture, and on-device Tesseract.js OCR. After capture, review fields for city + three words; **Confirm** fills `#decode_input` and calls `decode_input_from_form()`. **Retake** returns to live preview keeping mode. Fallbacks: **Use photo instead** (same review step), **Type instead**, or close and type/paste manually. |
| Icon caption overlay | Input Icon Guide | `body.decode-icon-guide`, `#decode_icon_guide_scrim`, `#map_icon_guide_dim`, `.decode_icon_caption`, `DecodeIconGuide.js`, `requestDecodeIconGuide()` | Vertically centered with the input cluster on Wolo Code Input View; chrome captions sit by Account, Info, Locate, Map, Wolo Code (right of icon on Map View), Switch map, Search, Go, and D-pad (left of the camera control) | Names each unlabeled icon on Wolo Code Input View for the first two launches (`localStorage` `wolo-decode-icon-guide-launches`). Light mode keeps the input-view background behind the labels; dark mode adds an 80% dark scrim. On Map View the same overlay auto-shows for the first two Map View openings (`wolo-map-icon-guide-launches`). A map-local dimmer keeps the complete camera D-pad and its caption above the overlay, draws `World-map.svg` over the input-view fill (teal land in light mode, brand-accent land in dark mode), and places `Wolo-infocard-sample.svg` as a sample Location Wolo Label View with leader-line callouts (larger title plus italic description) for City, Wolo Code, Address, Open, and Label; those lines stop short of the indicated spots in each direction. After an automatic Map View guide closes, `#notification_bottom` tells the user they can open it again from Info (i), then Show guide; that toast is skipped when Info Modal View launched the guide directly. Search and Go are lifted above the page scrim so they stay visible as solid themed fields, without an extra accent ring. OSM still uses the input-view fill rather than an 80% dark wash. Map captions are pinned in place without observing the live map DOM, so tile updates cannot freeze the tab. Search and Wolo Code input carets are hidden while the guide is showing. A tap anywhere, including the map, dismisses it, and it also fades out after `icon_guide_timeout` seconds from `Vars.tsv` (4). Hidden while a modal overlay is open, and held back on first launch until Info Intro Proceed. Info Modal View can reopen it with **show guide** on the current view. On very small screens the centered cluster can shift up so bottom notifications stay visible. |
| Map Guide search restoration | Map Search Cluster Restoration | `decodeIconGuideSearchHome.style`, `restoreMapSearchBarStacking()` | Google Maps top-left control position on wide and narrow layouts | Uses the search cluster's responsive CSS margins for its temporary fixed guide position rather than Google Maps' unsettled first-render rectangle. It preserves the complete SDK-provided inline style and restores that exact style when the guide closes. The dark-mode guide D-pad clone matches the live camera control's dark surface and accent icon. |
...
| Search field | Place Search Input | `#pac-input` | Top-left, inside `#map_search_bar` | Accepts place names and Wolo Codes while in the map flow and stays focused so typing can begin immediately, except during Input Icon Guide, when the field is blurred and the caret is hidden. CSS shows DIGIPIN and plus-code values in uppercase while the field is `:valid`; other input stays lowercase. |
...
| Label scan control | Map Scan Button | `#map_code_scan_button`, `#map_search_cluster` | Between Map Previous City Button and Place Search Input | Opens the same Wolo Code Scan Dialog as Wolo Code Input View. Success fills `#pac-input` and calls `decode_input_from_map()`. |
...
| Accuracy strip | Location Accuracy Indicator | `#accuracy_container`, `#wait_loader` | Inside `#map_bottom_stack`, above the Address Panel when both are open | Shows active location accuracy feedback while geolocation is running. The top bar loader stays continuous across initial bootstrap and IP-city retry attempts, and during location work it shows until the first GPS fix before hiding as the map zooms to the accuracy circle. Locate alone skips the shared final bounds correction so later fixes do not cause another camera bounce. |
...
| Locate proceed control | Location Proceed Button | `#proceed_button` | Right side of Location Accuracy Indicator, flush with the pill | Lets the user proceed with the current location fix. The control is vertically centered in the 32px pill, flattened onto the strip (no separate raised layer), and keeps its layout slot after it is hidden so the strip does not resize. While the meter is `99+`, locate samples once per second and auto-proceeds after five consecutive samples unless long-press locate override is on (`locate_button_pressed`). |
...
| Intro proceed control | Info Intro Proceed Button | `#info_intro_close_button` | Bottom of Info Intro View | Closes the intro prompt and continues into the app. First-run Input Icon Guide starts only after this click. |
...
| Icon labels control | Info Icon Labels Button | `#info_show_icon_labels`, `showInfoIconGuide()` | Bottom-left of Info Modal View | **show guide** closes Info Modal View and shows Input Icon Guide overlay labels on the current view (Wolo Code Input View or Map View). Hover and keyboard focus do not underline the label. Hidden during Info Intro View and Info Links View; shown on Info Full View. |
| Version indicator | Info Version Indicator | `#info_version_indicator`, `.info_version_under`, `toggleInfoVersionDisplay()` | Bottom-right of Info Modal View | Shows `v` plus the three-part `version` from `Vars.tsv` on the same bottom line as **show guide**. Hover or keyboard focus expands the label to `version.build` and shows elapsed time since the bake under the version. Hovering that stamp shows the UTC timestamp. A tap keeps the full version visible and shows local time under the version; hovering that stamp shows UTC. Times overlay below that line so the version stays put. Hidden on Info Links View. |
...
| Info author credit | Info Author Credit | `#info_agency` | Bottom of Info Modal View | Shows the creator credit. Visible on the first-run intro with the version number; on narrow viewports a right inset keeps a small gap between the name and the version. |

## Shared App Parts

| Part | Use this name | Current implementation signal | Typical position | Purpose |
| --- | --- | --- | --- | --- |
| Brand lockup | App Logo | `#logo`, `#logo_surface`, `#logo_wolo`, `#logo_codes` | Top-right | Persistent brand link visible across the app views. `#logo_surface` draws an 8px padded frosted rectangle around the Wolo + CODE lockup on map views, Wolo Code Input View, and the World-map icon-guide overlay, in light and dark mode. |
...
| View toggle action | Wolo Code Input Action | `#action_menu_decode`, `toggleDecodeView()` | Bottom-left on map views, after Info Action | Switches from map views to Wolo Code Input View. Hidden on Wolo Code Input View; use Terrain Map View Button instead. |
...
| Notifications | Notification Bars | `#notification_top`, `#notification_bottom` | Top-center and in `#map_bottom_stack` above the Address Panel | Shows short guidance, errors, and flow feedback. The bottom bar stacks with the Address Panel and Location Accuracy Indicator; the top bar stays below search. Choosing **Try: Bengaluru, India** zooms out first when already zoomed in, then pans and animates zoom-in to the sample Wolo Code. |
| Address panel | Address Panel | `#address_text` and Address fragment | Bottom of `#map_bottom_stack`, above Locate/footer | Shows and copies the resolved address for a selected Wolo Code. DIGIPIN (India) and plus code rows keep labels on the left and values on the right; plus code is encoded locally if geocoding omits it. Theme follows light/dark mode. Drag-selected text copies only the selection. Steering with a DIGIPIN or plus code keeps the panel open. Locate hides the panel. Small QR icons sit after the street address (bottom-right) and after each DIGIPIN and plus code (to the right) and open Code QR View. |
| Modal layer | Overlay Layer | `#overlay`, `onOverlayBackdropClick()` | Full viewport | Hosts info, redirects, city selection, add city, QR, account, crash, and browser-support dialogs. A click on the dimmed overlay outside the open dialog closes it, except Unexpected Error Dialog. |
| Add city dialog | Add City Modal View | `#no_city_message`, `#no_city_message_prompt`, `#no_city_message_wait` | Center overlay | Asks whether to add a location missing from the city database. Yes shows a wait spinner and continue/stop actions. Follows light and dark theme. Clicking the dimmed overlay outside the dialog closes it. |
| Unexpected error dialog | Unexpected Error Dialog | `#exception_message`, `#exception_message_title`, `#exception_log`, `#exception_log_copy`, `#exception_message_support` | Center overlay | Blocking crash prompt with a warning icon and `Oops an error occured!` title. No close button. First shows Clear cache & reload; the support email line appears only after that button was pressed and the crash returns. A successful load hides the contact again. Press and hold the title to show the crash message, a top-right Copy stack trace button inside the log window, and Continue. Clicking the dimmed overlay does not close it. |
| Info dialog | Info Modal View | `#info_message`, `#info_show_icon_labels` | Center overlay | Explains Wolo Code format, usage steps, and policy/contact details. The bottom-left **show guide** button closes the dialog and shows Input Icon Guide overlay labels on the current view. Clicking the dimmed overlay outside the dialog also closes it. |
| QR label dialog | QR Label View | `#qr_container` | Center overlay | Builds, previews, downloads, and prints a Wolo Code label. |
| Code QR dialog | Code QR View | `#code_qr_message`, `showCodeQR()` | Center overlay | Shows a QR for the Address Panel street address, DIGIPIN, or plus code, with that value under the code. Clicking the dimmed overlay outside the dialog closes it. |
| Account dialog | Account Address Book View | `#account_dialog_container`, `#account_dialog` | Center overlay | Shows profile, logout, appearance, map source prefs, and saved addresses. Clicking the dimmed overlay outside the dialog closes it. |
| Footer credit | Footer Credit | `#footer-content-container`, `#footer-content` | Bottom-center | Shows license year and author credit. |
| Map source credit | Map Source Attribution | `.map_attribution`, `.map_attribution_toggle`, MapKit logo on `#apple_map` | Bottom-left, close to the bottom edge after a gap from the bottom-left chrome; one line on widescreen. On mobile, a short credit at the bottom-left corner below the action icons sits on a fading gradient and expands to the full line on a dark bar. | Credits OSM, Esri, or Microsoft for non-Google map tiles. Apple Maps uses MapKit’s logo only (no extra Apple text). |
...
| Map Source Attribution | Bottom-left, close to the bottom edge after a gap from the bottom-left chrome; one line on widescreen | Bottom-left corner below the action icons; short credit on a fading gradient expands to the full line on a dark bar | `.map_attribution`, `Root_narrow.css`, `initMapAttribution()` |
| Info Action | Bottom-left on Wolo Code Input View; before Wolo Code Input Action on map views | Bottom-left on Wolo Code Input View; before Wolo Code Input Action on map views | `#action_menu_info` |
| Wolo Code Input Action | Bottom-left on map views, after Info Action | Bottom-left on map views, after Info Action | `#action_menu_decode` |
...
| Camera D-pad | Bottom-right, same 39px disc as Map Type Button, with a small gap to its left | Bottom-right, same 39px disc as Map Type Button, with a small gap to its left | `#map gmp-internal-camera-control`, `#map_camera_label` |

Chrome icon controls (Account, Go, Locate, Map Type, Info Action, Wolo Code Input Action, and city-source buttons) show a native tooltip from `title`. The first two launches also show those icon captions on Wolo Code Input View after Info Intro Proceed, over an 80% dark scrim; a tap or the `icon_guide_timeout` from `Vars.tsv` (4 seconds) fades the overlay away. The first two Map View openings show the Map View guide; closing an automatic showing leaves a bottom toast that the guide is available from Info (i), then Show guide. Info Modal View can show that overlay again with **show guide**, including on Map View. Appearance theme buttons omit native tooltips; hovering or focusing shrinks the icon so the System/Light/Dark label can appear. Light-mode theme icons stay gray instead of using the primary accent; dark-mode hover uses a white/gray highlight instead of a filled primary-accent button.
