...
| Version indicator | Info Version Indicator | `#info_version_indicator`, `.info_version_under`, `toggleInfoVersionDisplay()` | Bottom-right of Info Modal View | Shows `v` plus the three-part `version` from `Vars.tsv` on the same bottom line as **show guide**. Hover or keyboard focus expands the label to `version.build` and shows the UTC timestamp under the version. Hovering that timestamp shows elapsed time since the bake. A tap keeps the full version visible and shows local time under the version; hovering that stamp also shows elapsed time. Times overlay below that line so the version stays put. Hidden on Info Links View. |
