...
| Unexpected error dialog | Unexpected Error Dialog | `#exception_message`, `#exception_message_title`, `#exception_log`, `#exception_message_support` | Center overlay | Blocking crash prompt with a warning icon and `Oops an error occured!` title. No close button. Shows Clear cache & reload, then “You may contact our support team.” and a crawl-proof support email on the next line. Press and hold the title to show the crash message and Continue. |
